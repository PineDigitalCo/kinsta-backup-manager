...
final class Kinsta_BM_Admin {
	...
	private function handle_save_settings(): void {
		...
		if ( isset( $_POST['kinsta_bm_default_notify_user_id'] ) && is_string( $_POST['kinsta_bm_default_notify_user_id'] ) ) {
			update_option(
				'kinsta_bm_default_notify_user_id',
				sanitize_text_field( wp_unslash( $_POST['kinsta_bm_default_notify_user_id'] ) ),
				false
			);
		}
	...
	private function render_settings_tab(): void {
		...
		echo '<tr><th scope="row">' . esc_html__( 'Default notify user (restore)', 'kinsta-backup-manager' ) . '</th><td>';
		if ( ! empty( $users ) ) {
			echo '<select name="kinsta_bm_default_notify_user_id">';
			echo '<option value="">' . esc_html__( '— Select —', 'kinsta-backup-manager' ) . '</option>';
			foreach ( $users as $u ) {
				printf(
					...
					esc_attr( $u['id'] ),
					esc_html( $u['name'] ),
					selected( $u['id'], $notify_id, false ),
					esc_html( $u['email'] )
				);
			}
			echo '</select>';
		} else {
			echo '<input type="text" class="regular-text" name="kinsta_bm_default_notify_user_id" value="' . esc_attr( $notify_id ) . '" placeholder="' . esc_attr__( 'User UUID', 'kinsta-backup-manager' ) . '" />';
		}
		echo '<p class="description">' . esc_html__( 'Kinsta requires a company user ID for restore notifications.', 'kinsta-backup-manager' ) . '</p>';
		echo '</td></tr>';
	...
	private function render_backups_tab(): void {
		...
		$default_notify = (string) get_option( 'kinsta_bm_default_notify_user_id', '' );
			...
			if ( ! empty( $notify_users ) ) {
				...
				foreach ( $notify_users as $u ) {
					printf(
						...
						selected( $u['id'], $default_notify, false )
					);
				}
				echo '</select></label></p>';
			} else {
				echo '<input type="text" class="regular-text" name="kinsta_bm_notify_user_id" value="' . esc_attr( $default_notify ) . '" placeholder="' . esc_attr__( 'User UUID', 'kinsta-backup-manager' ) . '" /></label></p>';
			}
			if ( current_user_can( 'kinsta_bm_restore_live' ) ) {
